...
class DrawPanel extends JPanel implements ActionListener, MouseListener {
    ...
    protected void paintComponent(Graphics g) {
        ...
        int [] temp = animationCord;

        g.drawImage(backCard.getImage(), 300, 100, null);

        if (d.getDeck().isEmpty()){
            for (int i = 1; i < missingCards.size(); i++){
                missingCards.remove(i);
            }
            missingCards.add(new int[] {99999,0});
        }
        if (cardAnimation){
            g.drawImage(backCard.getImage(), temp[0], temp[1], null);
        } else if (turnAniStart || missingCards.size() > 0){
            Graphics2D tempG2d = (Graphics2D) g2d.create();
            AffineTransform old = tempG2d.getTransform();
            tempG2d.translate(temp[0] + 40, temp[1]);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (!missingCards.isEmpty() && missingCards.getFirst()[0] != 99999){
            cardAnimation = true;
            ...
            if (cordGoal[0] == animationCord[0] && cordGoal[1] == animationCord[1]) {
                cardAnimation = false;
                if (cards[missingCards.getFirst()[0]][missingCards.getFirst()[1]].getValue().equals("0")) {
                    turnAniStart = true;
                }
                if (turnAniStart){
                    if (!scaleDirection) {
